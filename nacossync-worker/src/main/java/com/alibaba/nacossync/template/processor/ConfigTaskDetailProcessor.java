...
@Slf4j
@Service
public class ConfigTaskDetailProcessor implements Processor<ConfigTaskDetailQueryRequest, ConfigTaskDetailQueryResult> {

    private final ConfigTaskAccessService configTaskAccessService;

    public ConfigTaskDetailProcessor(ConfigTaskAccessService configTaskAccessService) {
        this.configTaskAccessService = configTaskAccessService;
    }
    
    @Override
    public void process(ConfigTaskDetailQueryRequest configTaskDetailQueryRequest, ConfigTaskDetailQueryResult configTaskDetailQueryResult,
            Object... others) throws Exception {
        
        ConfigTaskDO taskDO = configTaskAccessService.findByTaskId(configTaskDetailQueryRequest.getTaskId());
        
        if (null == taskDO) {
            throw new SkyWalkerException("configTaskDo is null,taskId :" + configTaskDetailQueryRequest.getTaskId());
        }

        configTaskDetailQueryResult.setConfigTaskModel(ConfigTaskModel.from(taskDO));
    }
}
